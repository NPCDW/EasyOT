...
pub fn screenshot(app_handle: AppHandle) -> (Vec<u8>, i32, i32, u32, u32, f64) {
    ...
    for (i, monitor) in monitors.iter().enumerate() {
        if monitor.position().x <= mouse.0
            && mouse.0 < monitor.position().x + monitor.size().width as i32
            && monitor.position().y <= mouse.1
            && mouse.1 < monitor.position().y + monitor.size().height as i32
        {
            index = i + 1;
